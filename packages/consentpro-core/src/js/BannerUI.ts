...
export class BannerUI {
  ...
  init(containerId: string, config: BannerConfig): void {
    ...
    document.dispatchEvent(new CustomEvent('consentpro_ready', { detail: { config } }));
  }
  ...
  private _renderLayer1(): void {
    ...
    this._container.innerHTML = `<div class="consentpro__container"><div class="consentpro__content"><h2 id="consentpro-heading" class="consentpro__heading">${this._escapeHtml(text.heading)}</h2><p id="consentpro-description" class="consentpro__description">${this._escapeHtml(text.description)} <a href="${this._escapeHtml(policyUrl)}" class="consentpro__link" target="_blank" rel="noopener">Learn more</a></p></div><div class="consentpro__actions" role="group" aria-label="Cookie consent options"><button type="button" class="consentpro__btn consentpro__btn--secondary" data-action="reject">${this._escapeHtml(text.rejectNonEssential)}</button><button type="button" class="consentpro__btn consentpro__btn--secondary" data-action="settings">${this._escapeHtml(text.settings)}</button><button type="button" class="consentpro__btn consentpro__btn--primary" data-action="accept">${this._escapeHtml(text.acceptAll)}</button></div><div class="visually-hidden" aria-live="polite" aria-atomic="true" id="consentpro-live-region"></div></div>`;
  ...
  private _renderLayer2(): void {
    ...
    this._container.innerHTML = `<div class="consentpro__container"><header class="consentpro__settings-header"><button type="button" class="consentpro__back" aria-label="${this._escapeHtml(text.back)}" data-action="back"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></button><h2 id="consentpro-settings-title" class="consentpro__settings-title">${this._escapeHtml(settingsTitle)}</h2></header><p id="consentpro-settings-desc" class="visually-hidden">Manage your cookie preferences by category. Essential cookies are always enabled.</p><ul class="consentpro__categories" role="list">${categories.map((cat) => this._renderCategory(cat)).join('')}</ul><footer class="consentpro__settings-footer"><a href="${this._escapeHtml(policyUrl)}" class="consentpro__policy-link" target="_blank" rel="noopener noreferrer">Privacy Policy</a><button type="button" class="consentpro__btn consentpro__btn--primary" data-action="save">${this._escapeHtml(text.save)}</button></footer><div class="visually-hidden" aria-live="polite" aria-atomic="true" id="consentpro-live-region"></div></div>`;
  ...
  private _renderCategory(category: CategoryDefinition): string {
    ...
    return `<li class="consentpro__category"><div class="consentpro__category-info"><div class="consentpro__category-header"><span class="consentpro__category-name" id="${labelId}">${this._escapeHtml(category.name)}</span>${isRequired ? '<span class="consentpro__category-required">(Always active)</span>' : ''}</div><p class="consentpro__category-desc" id="${descId}">${this._escapeHtml(category.description)}</p></div><button type="button" role="switch" class="consentpro__toggle" aria-checked="${isChecked}" aria-labelledby="${labelId}" aria-describedby="${descId}"${isRequired ? ' disabled' : ''} data-category="${category.id}"></button></li>`;
  }
  ...
  /**
   * Save consent and close banner
   */
  private _saveAndClose(categories: Partial<ConsentCategories>): void {
    this._manager.setConsent(categories);
  ...
  renderFooterToggle(containerId?: string): void {
    ...
    this._footerToggle.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg><span>${this._escapeHtml(text)}</span>`;
  ...
  /**
   * Set up keyboard handlers (Escape to close, Tab focus trap)
   */
  ...
  private _getFocusableElements(): HTMLElement[] {
    return this._container
      ? Array.from(this._container.querySelectorAll<HTMLElement>('button:not([disabled]),[href]'))
      : [];
  }
  ...
  private _returnFocus(): void {
    if (this._triggerElement instanceof HTMLElement) {
      requestAnimationFrame(() => (this._triggerElement as HTMLElement).focus());
    }
  }
